<template>
    ...
    <div class="half-width">
      ...
      <div class="body-primary">
        <div class="commodityListBox">
          <div class="d-flex flex-row justify-content-center pt-1 mb-4">
            <nuxt-link :to="`/sharer/round/commodities/add/#!/update/${sharingRoundPayload.id}/`">
              <b-btn class="btn sameBtnWithPrimary ">Add More commodities</b-btn>
            </nuxt-link>
            <div class=" p-2 text-left badge color-orange ml-2   border border-warning ">
              <table>
                <tr>
                  <td>
                    <i class="fas fa-exclamation-triangle fa-2x"></i>
                  </td>
                  <td>Add new item(s) before <br/> editing previous item(s).</td>
                </tr>
              </table>
            </div>
          </div>
  ...
  <div
    v-if="!sharingRoundPayload.isPublished"
    class="text-center"
  >
...
export default {
  ...
  methods: {
    ...
    async deleteItem() {
      ...
      }/${this.sharingRoundPayload.id}/commodities/${this.itemToDelete}`
      await this.$axios
        .$delete(URI, {})
        .then(res => {
